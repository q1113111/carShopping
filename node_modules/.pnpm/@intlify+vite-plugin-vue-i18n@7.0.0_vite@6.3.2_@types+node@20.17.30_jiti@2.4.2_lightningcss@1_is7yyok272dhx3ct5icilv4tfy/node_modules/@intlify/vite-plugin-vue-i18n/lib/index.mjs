...
import { isArray, isString, isBoolean, isEmptyObject, generateCodeFrame } from '@intlify/shared';
...
function pluginI18n(options = { forceStringify: false }) {
  ...
  const runtimeModule = `${installedVueI18nBridge ? "vue-i18n-bridge" : installedPkg}.runtime.mjs`;
  ...
  return {
    ...
    config(config, { command }) {
      if (command === "build" && runtimeOnly) {
        ...
        if (isArray(config.resolve.alias)) {
          ...
          config.resolve.alias[getAliasName()] = `${installedPkg}/dist/${runtimeModule}`;
        }
        ...
        if (isArray(config.resolve.alias)) {
          ...
          config.resolve.alias["vue-i18n"] = `petite-vue-i18n/dist/petite-vue-i18n.mjs`;
        }
        debug(`alias name: ${getAliasName()}`);
      }
    ...
    async resolveId(id, importer, options2) {
      if (options2?.ssr) {
        ...
        if (id.endsWith(runtimeModule) && importer.toString().endsWith(INTLIFY_FEATURE_PROXY_SUFFIX)) {
          return null;
        }
